...
export function DealDetailClient({
  ...
  // Generate brief client-side when briefPending flag is set
  const generateBriefClientSide = async () => {
    if (briefGenerating || !currentUser) return;
    ...
    try {
      ...
      console.error("[deal-detail] Client-side brief generation error:", err);
    } finally {
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2 shrink-0 ml-4">
            <button
              ...
              disabled={callPrepPhase === "loading" || briefGenerating}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-all"
              style={{
                background: briefReady ? "rgba(224,122,95,0.15)" : briefGenerating ? "rgba(224,122,95,0.12)" : callPrepPhase === "error" ? "rgba(199,75,59,0.1)" : callPrepPhase !== "hidden" ? "rgba(224,122,95,0.12)" : "#F3EDE7",
                color: briefReady ? "#E07A5F" : briefGenerating ? "#E07A5F" : callPrepPhase === "error" ? "#C74B3B" : callPrepPhase !== "hidden" ? "#E07A5F" : "#8A8078",
                border: "1px solid " + (briefReady ? "rgba(224,122,95,0.4)" : briefGenerating ? "rgba(224,122,95,0.3)" : callPrepPhase === "error" ? "rgba(199,75,59,0.3)" : callPrepPhase !== "hidden" ? "rgba(224,122,95,0.3)" : "#E8E5E0"),
              }}
            >
