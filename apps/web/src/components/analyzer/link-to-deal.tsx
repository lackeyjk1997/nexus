...
import { X, Search, Check, ExternalLink, Building2, Target, Unlink } from "lucide-react";
...
export function LinkToDeal({
  ...
  async function linkCompany(company: Company) {
    setSaving(true);
    try {
      const res = await fetch("/api/analyze/link", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ companyId: company.id, analysis }),
      });
      if (res.ok) {
        setLinkedCompanyId(company.id);
        setLinkedCompanyName(company.name);
        setOpen(false);
      }
    } finally {
      setSaving(false);
  ...
  function handleUnlink() {
    ...
    setLinkedDealName(null);
    setLinkedCompanyId(null);
    setLinkedCompanyName(null);
  }

  const isLinked = linkedDealId || linkedCompanyId;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-foreground">
              Analysis linked to {linkedDealName || linkedCompanyName}
            </p>
            <p className="text-xs text-muted-foreground">
              Saved to the {linkedDealId ? "deal" : "company"}&apos;s activity timeline
            </p>
          </div>
        </div>
  ...
  return (
    <>
      ...
      >
        Link this analysis to a deal or company
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-card rounded-xl border border-border shadow-xl w-full max-w-md max-h-[80vh] flex flex-col">
            ...
            <div className="px-4 pt-3 flex gap-2">
              <button
                onClick={() => { setMode("deal"); setSearch(""); }}
                className={cn(
                  "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors",
                  mode === "deal" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground"
                )}
              >
                <Target className="h-3 w-3" /> Link to Deal
              </button>
              <button
                onClick={() => { setMode("company"); setSearch(""); }}
                className={cn(
                  "flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors",
                  mode === "company" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:text-foreground"
                )}
              >
                <Building2 className="h-3 w-3" /> Link to Company
              </button>
            </div>
            ...
            <div className="p-4 border-b border-border">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={mode === "deal" ? "Search deals..." : "Search companies..."}
                ...
                filteredCompanies.map((company) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground capitalize">{company.industry.replace("_", " ")}</p>
                    </div>
                  </button>
                ))
              )}
              {((mode === "deal" && filteredDeals.length === 0) || (mode === "company" && filteredCompanies.length === 0)) && (
                <p className="text-sm text-muted-foreground text-center py-8">
                  No {mode === "deal" ? "deals" : "companies"} found
                </p>
              )}
            </div>
          </div>
